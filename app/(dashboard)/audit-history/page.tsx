        ...
        <div className="space-y-6 pb-20 md:pb-0">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500">View past audit records and reports.</p>
                </div>
            </div>
            ...
            >
                {/* Filters */}
                <div className="hidden md:block rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                <div className="relative flex-1 min-w-0">
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-3 py-2 text-sm text-slate-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none"
                                        placeholder="From"
                                    />
                                </div>
                                ...
                                {(startDate || endDate) && (
                                    <button
                                        ...
                                        className="w-full sm:w-auto p-2 sm:px-3 sm:py-2 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-red-500 transition-colors flex items-center justify-center cursor-pointer shadow-sm active:scale-95 bg-white"
                                        title="Clear Dates"
                                    >
                                        <X className="h-4 w-4" />
                                    </button>
                        ...
                        <button
                            ...
                            className="w-full md:w-auto flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/20 transition-all hover:shadow-emerald-500/40 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                        >
                        ...
                        filteredLogs.map((log) => (
                            ...
                            >
                                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                    ...
                                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${log.status === "Completed"
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        {!log.supervisor1VerifiedBy && user && ["Suradet Sarnyos", "Phakkhawat Khamkon"].includes(user.name) && (
                                            <button
                                                ...
                                                className="flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-green-700 hover:shadow-md transition-all active:scale-95 cursor-pointer"
                                            >
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100 hover:text-blue-800 transition-colors cursor-pointer"
                                        >
                            ...
                            <thead className="bg-slate-50/80 backdrop-blur supports-[backdrop-filter]:bg-slate-50/80 sticky top-0 z-10">
                                <tr>
                                    {[
                                        ...
                                        { id: "total", label: "Total Asset" },
                                        { id: "found", label: "Found" },
                                        { id: "missing", label: "Missing" },
                                        { id: "status", label: "Status" },
                                        { id: "supervisor1", label: "Supervisor Check" },
                                        { id: "actions", label: "Actions" },
                                    ].map((col) => (
                                        <th
                                            key={col.id}
                                            className="relative px-6 py-4 text-left text-sm font-semibold text-slate-700 select-none group overflow-hidden text-ellipsis whitespace-nowrap"
                                ...
                                {filteredLogs.length === 0 ? (
                                    ...
                                    filteredLogs.map((log) => (
                                        ...
                                        >
                                            ...
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-700">{log.auditedBy || "-"}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-semibold text-slate-900">{log.totalAssets}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-bold text-green-600 bg-green-50/50">{log.scannedCount}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-bold text-red-600 bg-red-50/50">{log.missingCount}</td>
                                            <td className="whitespace-nowrap px-6 py-4">
                                                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold ${log.status === "Completed"
                                                    ? "bg-green-100 text-green-700 border border-green-200"
                                                    : "bg-amber-100 text-amber-700 border border-amber-200"
                                                    }`}>
                                                    {log.status === "Completed" ? (
                                            ...
                                            <td className="whitespace-nowrap px-6 py-4 text-sm">
                                                {log.supervisor1VerifiedBy ? (
                                                    <div className="flex items-center gap-1.5 text-green-600 font-medium">
                                                        ...
                                                        <span>{log.supervisor1VerifiedBy}</span>
                                                    </div>
                                                ) : (
                                                    <div className="flex items-center gap-1.5 text-amber-500">
                                                        ...
                                                        {user && ["Suradet Sarnyos", "Phakkhawat Khamkon"].includes(user.name) && (
                                                            <button
                                                                onClick={() => handleVerify(log)}
                                                                className="ml-2 rounded-lg bg-green-600 px-2 py-0.5 text-xs text-white hover:bg-green-700 cursor-pointer"
                                            ...
                                            <td className="whitespace-nowrap px-6 py-4 text-sm">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button
                                                        ...
                                                        className="rounded-lg p-1.5 text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer"
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100 cursor-pointer"
